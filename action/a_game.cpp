...
void PrintMOTD(edict_t * ent)
{
	...
	strcpy(msg_buf, TNG_TITLE " v" VERSION "\n" "http://github.com/actionquake/aq2-tng" "\n\n");
	lines = 3;
	...
	if (!skipmotd->value) {
		...
		else  // So it's not Teamplay?
		{
			...
			/// Q2R: No support for DM Teams, use TeamDM for this
		 ...
		 */
		if (teamplay->value && !g_friendly_fire->value) {
	...
	if (!auto_menu->value || ent->client->pers.menu_shown) {
		gi.LocClient_Print(ent, "%s", msg_buf);
	} else {
		gi.LocCenter_Print(ent, PRINT_LOW, "%s", msg_buf);
	}
}
...
// 	blooder->nextthink = level.time + 300_ms;
...
// 	shell->nextthink = level.time + (shelllife->value - (shells * 0.05)) * 40_hz;
...
// 		VectorScale( self->movetarget->velocity, level.time + FRAME_TIME_S, self->s.origin );
// 		VectorAdd( self->movetarget->s.origin, self->s.origin, self->s.origin );
// 		VectorScale( self->movetarget->avelocity, level.time + FRAME_TIME_S, self->s.angles );
