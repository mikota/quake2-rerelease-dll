...
//// Replaced with enum logic, remove
...
bool visible(edict_t* self, edict_t* other, int mask);
...
void fire_blaster(edict_t* self, vec3_t start, vec3_t aimdir, int damage,
	int speed, int effect, bool hyperb);
...
bool SV_FilterPacket(char* from, int* temp);
bool SV_FilterSBPacket(char* from, int* temp); //rekkie -- silence ban
